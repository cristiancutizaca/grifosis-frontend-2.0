...
function useEmployeeReport({ userId, userIdsCsv, dateRange, onlyCompleted }: Required<Pick<Props, 'dateRange'>> & Pick<Props, 'userId' | 'userIdsCsv' | 'onlyCompleted'>) {
  ...
  const loadInventory = useCallback(async () => {
    if (!isMulti) {
      ...
      return setInventory(normalizeInventory(data?.rows || data || []))
    }
    const ids = (userIdsCsv || '').split(',').map(s => s.trim()).filter(Boolean)
    const results = await Promise.allSettled(
      ids.map(id => fetchJson(`${base}/reports/inventory/movements-ui?${qs({ userId: id, startDate: dateRange.startDate, endDate: dateRange.endDate })}`))
    )
    const merged = results.flatMap(r => r.status === 'fulfilled' ? (r.value?.rows || r.value || []) : [])
    setInventory(normalizeInventory(merged))
  ...
  const loadShifts = useCallback(async () => {
    ...
    console.log('by-shift resp', data);
...
function useSalesAgg(timeseries: Array<{ date: string; gross: number }>, sales: SaleDetailRow[]) {
  return useMemo(() => {
    const byDay = timeseries.length
      ? timeseries.map((p) => ({ date: p.date, amount: Number(p.gross || 0) }))
      : Array.from(
        sales.reduce((m, r) => m.set(r.date?.substring(0, 10) || '', (m.get(r.date?.substring(0, 10) || '') || 0) + Number(r.total_amount || 0)), new Map<string, number>()),
        ([date, amount]) => ({ date, amount }),
      ).sort((a, b) => (a.date < b.date ? -1 : 1))

    const uniq = new Map<number, { sale_id: number; date: string; seller: string; user_id?: number; amount: number; payment?: string }>()
    for (const r of sales) {
      if (r.sale_id == null) continue
      const key = Number(r.sale_id)
      if (!uniq.has(key)) {
    ...
    const bySeller = Array.from(uniqueArr.reduce((m, u) => {
      const k = u.seller
      const p = m.get(k) || { seller: k, amount: 0, transactions: 0 }
      m.set(k, { seller: k, amount: p.amount + u.amount, transactions: p.transactions + 1 })
      return m
    }, new Map<string, { seller: string; amount: number; transactions: number }>()).values()).sort((a, b) => b.amount - a.amount)

    const byProduct = Array.from(sales.reduce((m, r) => m.set(r.product_name || '—', (m.get(r.product_name || '—') || 0) + Number(r.item_subtotal || 0)), new Map<string, number>()), ([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value)

    const topSeller = bySeller[0]
    const largestSale = uniqueArr.reduce((mx, u) => (!mx || u.amount > mx.amount ? u : mx), undefined as undefined | typeof uniqueArr[number])

    return { uniqueArr, bySeller, byDay, byProduct, topSeller, largestSale }
  }, [timeseries, sales])
}
...
const EmployeeDetailedReport: React.FC<Props> = ({ userId, userName, userIdsCsv, dateRange, onlyCompleted = true }) => {
  ...
  const salesAgg = useSalesAgg(timeseries, sales)
          ...
          {activeTab === 'sales' && (
            <div className="space-y-5">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="bg-slate-700 rounded-lg p-4 border border-slate-600"><h5 className="font-semibold text-white mb-2">Mayor venta</h5>{salesAgg.largestSale ? (<><div className="text-white text-lg font-semibold">{fmtMoney(salesAgg.largestSale.amount)}</div><div className="text-slate-400 text-sm">{salesAgg.largestSale.seller} • {fmtDate(salesAgg.largestSale.date)}</div></>) : (<div className="text-slate-400 text-sm">Sin datos.</div>)}</div>
                {!isMulti && (
